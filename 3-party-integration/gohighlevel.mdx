---
title: "Integrate  TelaviOneChatAI with GoHigh Level"
description: "This is the guide for integrating  TelaviOneChatAI with GoHighLevel Integration"
---
...
- **Streamlined Lead Management** – Capture and sync  TelaviOneChatAI leads directly into GoHighLevel’s CRM for centralized tracking.
- **Automated Follow-Ups** – Trigger SMS, email, and voicemail campaigns based on chatbot interactions without manual intervention.
- **Unified Customer Journey** – Combine GoHighLevel funnels and  TelaviOneChatAI conversations for a seamless engagement flow.
- **Smart Workflow Automation** – Use  TelaviOneChatAI chat events to initiate GoHighLevel workflows and nurture sequences.

---

## **Steps for Integration:**

#### **Step 1. Navigating to GHL in  TelaviOneChatAI:**

1. Log into your  TelaviOneChatAI account, and go to your desired bot. Click on “**Third-party Apps Integrations**” option to go to all available 3rd Party Integrations page.
2. Go to GoHighLevel CRM integration either by:
...
5. The following page will open up, where you will have to enter the API Token for the connection to be established.
...
2. Once you have connected successfully, the below screen will appear. The following details needs to be filled in the same.
3. **Step 4: Set Up Mapping Parameters**

   Now that the connection between  TelaviOneChatAI and GoHighLevel (GHL) CRM is established, you need to map parameters (fields) of GHL CRM to the user responses from  TelaviOneChatAI's chatbot.
   1. You can use the **\+Add Parameters** button to add more fields.

      <Info>
        **Example Mapping Process**
        1. **Select a Question from  TelaviOneChatAI Chat Flow**
...
2. As a result, this mapping will enable the integration to **send the user's response from  TelaviOneChatAI to GoHighLevel's CRM fields**, storing it correctly in the contact records.
